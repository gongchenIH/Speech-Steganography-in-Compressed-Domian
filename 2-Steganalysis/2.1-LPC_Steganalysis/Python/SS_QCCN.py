...
NUM_PCA_FEATURE = 3   # number of PCA features
NUM_SAMPLE = 5       # total number of samples used for training
...
def main(positive_data_folder, negative_data_folder, result_folder):
    ...
    print("Extracting LPC Feature..")
    lpc_model = G729_SS_QCCCN
    ...
    for i in tqdm(range(num_train_files)):
        new_feature = lpc_model(train_negative_data_files[i])
        feature.append(new_feature)
    for i in tqdm(range(num_train_files)):
        new_feature = lpc_model(train_positive_data_files[i])
        feature.append(new_feature)
    ...
    Y = []
    for i in tqdm(range(num_train_files)):
        lpc_feature = lpc_model(train_negative_data_files[i])
        X.append(pca.transform(lpc_feature.reshape(1, -1)))
        Y.append(0)
    for i in tqdm(range(num_train_files)):
        lpc_feature = lpc_model(train_positive_data_files[i])
        X.append(pca.transform(lpc_feature.reshape(1, -1)))
        Y.append(1)
    X = np.row_stack(X)
    ...
    for i in tqdm(range(num_test_files)):
        lpc_feature = lpc_model(test_negative_data_files[i])
        X.append(pca.transform(lpc_feature.reshape(1, -1)))
        Y.append(0)
    for i in tqdm(range(num_test_files)):
        lpc_feature = lpc_model(test_positive_data_files[i])
        X.append(pca.transform(lpc_feature.reshape(1, -1)))
        Y.append(1) 
    X = np.row_stack(X)
...
if __name__ == "__main__":
    main('.\Cover_LPC', '.\Stego_LPC', '.\ResultLPC')
